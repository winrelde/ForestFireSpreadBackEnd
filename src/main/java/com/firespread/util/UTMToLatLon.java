...
public class UTMToLatLon {
    ...
    public LatAndLogEntity UTMXYToLatLon(double x, double y, int zone, boolean southhemi)
    {
        double cmeridian;
        ...
        LatAndLogEntity latAndLogMedium = MapXYToLatLon(x, y, cmeridian);
        latAndLogMedium.setLat(RadToDeg(latAndLogMedium.getLatMedium()));
        latAndLogMedium.setLog(RadToDeg(latAndLogMedium.getLogMedium()));
        return latAndLogMedium;
    }
    ...
    public LatAndLogEntity MapXYToLatLon(double x, double y, double lambda0)
    {
        ...
        /* Calculate latitude */
        double latMedium = phif + x2frac * x2poly * (x * x) + x4frac * x4poly * pow(x, 4.0) + x6frac * x6poly * pow(x, 6.0) + x8frac * x8poly * pow(x, 8.0);

        /* Calculate longitude */
        double logMedium = lambda0 + x1frac * x + x3frac * x3poly * pow(x, 3.0) + x5frac * x5poly * pow(x, 5.0) + x7frac * x7poly * pow(x, 7.0);
        latAndLog.setLatMedium(latMedium);
        latAndLog.setLogMedium(logMedium);
        return latAndLog;
    }
